...
public class CommentsServ extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response)
		...
		String cmd = request.getParameter("cmd");  //cmd파라미터 값으로 insert, update등 작업할건지 넘겨주는 걸로 결정됨
		...
		if (cmd == null) {
			...
			sb.append("]]></data>");
			sb.append("</result>");
			out.print(sb.toString());  //out.print는 웹페이지 표출 (out을 response.getWriter();로 선언)

		} else if (cmd.equals("selectAll")) { //하면
			try {
				String no = request.getParameter("no"); // 게시글번호를 받아옴
				...
				out.print(toXML("error", msg));  ////처리하다가 예외발생하면
			}
	...
	private String toXML(String result, HashMap data) throws IOException {  //(결과값은 , 데이터)
		StringBuffer sb = new StringBuffer();
		sb.append("<result>");
		sb.append("<code>");
		sb.append(result);
